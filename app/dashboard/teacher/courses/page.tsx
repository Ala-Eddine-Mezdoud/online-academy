...
export default function CoursesPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [coursesData] = await Promise.all([getMyCourses()]);
      ...
      <div className="bg-white rounded-lg ">
        <div className="text-black grid grid-cols-1 md:grid-cols-2 gap-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          {courses.map((course) => (
            <div
              key={course.id}
              className="relative flex flex-col justify-between bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 ease-in-out overflow-hidden border border-gray-300 p-4 "
            >
              <Link
              ...
              <div className="p-4 border-t border-gray-200 flex gap-2">
                <Link
                  href={`/dashboard/teacher/courses/${course.id}/live`}
