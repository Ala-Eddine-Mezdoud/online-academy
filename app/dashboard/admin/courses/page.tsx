...
import { courses as initialCourses, teachers as initialTeachers, Course, NewCourse, Teacher, students } from '@/app/lib/mockData';
...
export default function CoursesPage() {
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const matchesFilter = filterDepartment === 'all' || (course.department ?? '') === filterDepartment;
    return matchesSearch && matchesFilter;
  });

  const handleAddCourse = (courseData: NewCourse) => {
    const nextId = courses.length ? Math.max(...courses.map(c => c.id)
) + 1 : 1;
      ...
      <div className="p-8"> 
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="px-6 py-4 border-b border-gray-200">
              ...
              <div>
                <h3 className="text-2xl font-bold text-slate-900 mb-2">All Courses</h3>
                <p className="text-xs text-gray-500">Comprehensive list of all courses offered.</p>
              </div>
              <button 
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-2 text-left text-sm font-semibold text-slate-900">Course Code</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredCourses.map((course) => (
                  <tr key={course.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 table-text">{course.department ?? '-'}</td>
                    <td className="px-4 py-2 table-text">{course.assignedTeacher ?? '-'}</td>
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Add New Course">
        ...
        >
          <div>
            ...
            <input name="title" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
          </div>
          <div>
            <label className="block text-sm mb-2">Course Code</label>
            <input name="code" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="assignedTeacher" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={() => setIsAddModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm">Cancel</button>
            <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm">Add Course</button>
          </div>
        </form>
      </Modal>
    </div>
